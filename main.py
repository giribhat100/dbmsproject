...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/irqsql'
db=SQLAlchemy(app)
...
def report():
    if request.method=="POST":
        ...
        query=db.engine.execute(f"SELECT * FROM `student` WHERE `student`.`sem`={sem}") 
        return render_template('report.html',query=query,bio=query)
...
def edit(id):
    ...
    if request.method=="POST":
        rollno=request.form.get('rollno')
        if not(len(re.search(r"^[0-9][a-zA-z]{2}[0-9][0-9][a-zA-z]{2}[0-9]{3}"),rollno)>0):
        ...
        if not(re.search(r"[0-9][a-zA-z]{2}[0-9][0-9][a-zA-z]{2}[0-9]{3}",rollno)):
            flash("invalid","danger")
            return redirect('/studentdetails')
...
def print():
    if request.method=="POST":
        sem=request.form.get('sem')
        con=connect(user="root",passwd="",host="localhost",database="students")
        df=sql.read_sql(f"SELECT `student`.*,`attendence`.`attendance` FROM `attendence`,`student` WHERE  `attendence`.`rollno`=`student`.`rollno` AND `student`.`sem`={sem}",con)
        df.to_excel('ds.xls')
        flash("xl sheet created","success")
